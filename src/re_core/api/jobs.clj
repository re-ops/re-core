...
(ns re-core.api.jobs
  ...
  (:require
    ...
    [re-core.common :refer (bad-req success import-logging get* link)]
    [gelfino.timbre :refer (get-tid)]
    [re-core.persistency [types :as t] [users :as u]]
    [re-core.persistency.actions :refer (find-action-for)]
    [re-core.jobs :as jobs :refer (enqueue)]
    [re-core.redis :refer (clear-locks)]
    [es.jobs :as es]
...
(defroutes- jobs {:path "/jobs" :description "Async job scheduling"}
  ...
  (POST- "/jobs/destroy/:id" [^:int id]
    {:nickname "destroySystem" :summary "System destruction job"
     :notes "Destroys a system, clearing it both from re-core's model storage and hypervisor"}
  ...
  (POST- "/jobs/clear/:id" [^:int id]
    {:nickname "clearSystem" :summary "System clearence job"
     :notes "Clears a system, only clearing it both from re-core's model storage not the hypervisor"}
