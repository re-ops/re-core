(ns re-core.integration.persistency.persmissions
  ...
  (:require
    ...
    [re-core.fixtures.data :refer (redis-ec2-spec redis-ec2-spec redis-type)]
...
(with-conf
  (against-background
    [(current-user) => {:identity "admin", :roles #{:re-core.roles/admin}, :username "admin"}]
    (with-state-changes [(before :facts (do (re-initlize) (add-users) (t/add-type redis-type)))]
      ...
      #_(fact "persmissionless access" :integration :redis :systems
            (let [id (s/add-system (assoc redis-ec2-spec :env :prod))]
             (s/get-system id) =>
               (throws ExceptionInfo (is-type? :re-core.persistency.systems/persmission-env-violation))
